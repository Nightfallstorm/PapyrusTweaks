...
void Settings::Experimental::Load(CSimpleIniA& a_ini)
{
	...
	detail::get_value(a_ini, bypassMemoryLimit, section, "bBypassMemoryLimit", ";Allows the Script Engine to use as much memory as needed, with no cap. This makes `iMaxAllocatedMemoryBytes` in Skyrim.ini useless\n;Note: Skyrim already bypasses memory limit when stressed, this setting just keeps it bypassed.");
}
