...
void Settings::MigrateDeprecatedTweaks(CSimpleIniA& a_ini)
{
	const char* section = "Tweaks";

	detail::get_value(a_ini, VMtweaks.maxOpsPerFrame, section, "iMaxOpsPerFrame", ";Maximum papyrus operations per frame. Higher number means better script performance on average\n;Has a very minor impact on framerate, and varies from script to script. (Default: 500, Vanilla value: 100). Recommended Range: 100-2000. Set to 0 to disable");
	detail::get_value(a_ini, loggertweaks.disableGetFormFromFile, section, "bDisableGetFormFromFileErrorLogs", ";Disables `File \" % s \" does not exist or is not currently loaded.` logs from being printed when calling Game.GetFormFromFile().\n;This only disables the logging of the error, the error itself will still occur");
	detail::get_value(a_ini, loggertweaks.improveBaseTypeMismatch, section, "bImproveBaseTypeMismatchLogs", ";Improves Base Type Mismatch error to show inheritance hierarchy of scripts; Also clarifies if the script was a genuine mismatch, or if the script doesn't exist");
	detail::get_value(a_ini, loggertweaks.improveValidateArgsErrors, section, "bImproveValidateArgsLogs", ";Improves several error logs relating to incompatible arguments to better clarify what is incompatible");
	detail::get_value(a_ini, loggertweaks.disableNoPropertyOnScript, section, "bDisableNoPropertyOnScriptErrorLogs", ";Disable \"Property %s on script %s attached to %s cannot be initialized because the script no longer contains that property\" log messages.\n;This only disables the logging of the warning, the warning itself will still occur");
	detail::get_value(a_ini, loggertweaks.disableMissingScriptError, section, "bDisableMissingScriptError", ";Disable \"Cannot open store for class \"%s\", missing file?\" errors being logged.\n;This only disables the logging of the error, the error itself will still occur");
	detail::get_value(a_ini, VMtweaks.stackDumpTimeoutThreshold, section, "iStackDumpTimeoutMS", ";Modify how long Papyrus can be \"overstressed\" before dumping stacks, in milliseconds (Default value: 15000, Vanilla value: 5000).\n;Set to 0 to disable the stack dump check, or -1 to disable this setting.\n;See https://www.nexusmods.com/skyrimspecialedition/articles/4625 for information on what a stack dump is");
	detail::get_value(a_ini, loggertweaks.summarizeStackDumps, section, "bSummarizeStackDumps", ";Adds a summary of events when dumping stacks to log");
	detail::get_value(a_ini, VMtweaks.enableDocStrings, section, "bEnableDocStrings", ";Enables loading of doc strings from scripts into the engine. Requires `bLoadDebugInformation` to be true in this INI");
	detail::get_value(a_ini, VMtweaks.enableDebugInfo, section, "bEnableDebugInformation", ";Enables Skyrim to load debug information of scripts, and print papyrus logs. This completely overrides bLoadDebugInformation in Skyrim.ini and is provided here for easier configuration");

	a_ini.Delete(section, nullptr, true);
	...
	detail::get_value(a_ini, experimental.speedUpNativeCalls, section, "bRunScriptsOnMainThreadOnly", ";Run scripts on main thread only, and desync most native function calls. This can drastically improve script performance, at a minor cost of framerate during heavy load.\n;The amount of performance and framerate cost is based on fMainThreadTaskletBudget, as well as the current script load");
	detail::get_value(a_ini, experimental.classesToExcludeFromSpeedUp, section, "sMainThreadClassesToExclude", ";(Requires bRunScriptsOnMainThreadOnly=true) List of script classes to exclude from being sped up by bRunScriptsOnMainThreadOnly.\n;It is strongly recommended to leave at defaults unless you absolutely know what you're doing");
	detail::get_value(a_ini, experimental.methodPrefixesToExcludeFromSpeedup, section, "sMainThreadMethodsToExclude", ";(Requires bRunScriptsOnMainThreadOnly=true) List of script method prefixes to exclude from being sped up (ex: \"Equip\" excludes \"EquipItem\" and \"EquipItemByID\", but does NOT exclude \"UnequipItem\".\n;It is strongly recommended to leave at defaults unless you absolutely know what you're doing");
...
{
	const char* section = "VM Tweaks (Tweaks that directly alter the script engine)";
...
{
	const char* section = "Logging Tweaks (Tweaks that affect script logging)";
